...
import { PHARMACY_LOCATIONS_MIN } from './pharmacy-locations';

export async function populatePharmaciesMin(keystone: any) {
  const pharmaDB = keystone.db.Pharmacy;
  const pharmaLocDB = keystone.db.PharmacyLocation;
  try {
    console.log(`----------------------------------------`);
    console.log(`🌱 Seeding [${PHARMACY_LOCATIONS_MIN.length}] Pharmacy Locations 🏥`);
    console.log(`----------------------------------------`);

    // eslint-disable-next-line no-restricted-syntax
    for (const pharmacyLocation of PHARMACY_LOCATIONS_MIN) {
      console.log(
      ...
      if (existing) {
        ...
        try {
          ...
          throw new Error(
            `Error creating populatePharmaciesMin [${pharmacyLocation.accreditationNumber}]`
          );
        }
      }
    }

    console.log(`✅ Seeded [${PHARMACY_LOCATIONS_MIN.length}] Pharmas 🏥`);
  } catch (error) {
    console.error('populatePharmaciesMin :: error', error);
  }
}
