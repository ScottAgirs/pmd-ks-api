...
export async function populateDummyUsers(keystone: any) {
  ...
  for (const userSeedData of dummyUsers) {
    console.log(
      "populateDummyUsers :: total dummyUsers",
      dummyUsers.length,
      " - current user",
      i
    );
    ...
    const createdSchedule =
      hasSchedule &&
      (await keystone.db.Schedule.createOne({
        data: {
          ...schedule,
          recurringSlots: !hasSchedule
            ? undefined
            : {
    ...
    const createdEvents =
      hasCalendar &&
      hasEvents &&
      (await keystone.db.CalendarEvent.createMany({
      ...
      : {
          ...
          calendar: {
            create: {
              ...
              events: {
                connect: [
                  ...createdEvents.map((event: any) => ({ id: event.id })),
                ],
              },
            },
          },
          ...
          doctorSubSpecialties: {
            connect: doctorSubSpecialties.map((sp: any) => ({
              value: sp.value,
            })),
          },
        };
    ...
    await keystone.db.User.createOne({
      ...
      .catch((err: any) =>
        console.log(
          `res :: err - FAILED TO CREATE A USER ${DUMMY_USERS[i].username}`,
          err
        )
      )
      // @ts-ignore
